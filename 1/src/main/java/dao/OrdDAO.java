...
public class OrdDAO {
	...
	public int getListCount(String start_date, String end_date, String min_price, String max_price) {
		...
		try {
			...
			System.out.println("getListCount(S,S,S,S) - SQL구문 오류 - " + e.getMessage());
			e.printStackTrace();
		} finally {
			close(rs);
			close(pstmt);
	...
	public List<OrdDTO> getOrderList(String start_date, String end_date, String min_price, String max_price, int ordPageNum, int listLimit) {
		...
		try {
			...
			System.out.println("getListCount(S,S,S,S,I,I) - SQL구문 오류 - " + e.getMessage());
			e.printStackTrace();
		} finally {
			close(rs);
			close(pstmt);
	...
	public List<OrdDTO> getOrderList(String start_date, String end_date, String min_price, String max_price, String order_status, int ordPageNum, int listLimit) {
		...
		try {
			...
			if(order_status.equals("yes")) {
				order_status = "결제완료";
				sql = "SELECT * FROM ord WHERE order_date>=? AND order_date<=? AND order_price>=? AND order_price<=? AND order_status=? ORDER BY order_num DESC LIMIT ?,?";
			} else if(order_status.equals("no")) {
				order_status = "결제취소";
				sql = "SELECT * FROM ord WHERE order_date>=? AND order_date<=? AND order_price>=? AND order_price<=? AND order_status=? ORDER BY order_num DESC LIMIT ?,?";
			}
			...
			System.out.println("getOrderList(S,S,S,S,S,I,I) - SQL구문 오류 - " + e.getMessage());
			e.printStackTrace();
		} finally {
			close(rs);
			close(pstmt);
